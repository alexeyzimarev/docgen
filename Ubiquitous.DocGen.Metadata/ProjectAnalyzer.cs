...
namespace Ubiquitous.DocGen.Metadata
{
    ...
    {
        readonly Buildalyzer.ProjectAnalyzer[] _analyzers;
        ...
        public static ProjectAnalyzer ForProjects(IEnumerable<string> projectFiles, ILoggerFactory loggerFactory = null)
        {
        ...
        public static ProjectAnalyzer ForSolution(string solutionFile, ILoggerFactory loggerFactory = null)
        {
            var log     = loggerFactory ?? NullLoggerFactory.Instance;
            var manager = new AnalyzerManager(solutionFile, new AnalyzerManagerOptions {LoggerFactory = log});
        ...
        async Task<(string File, Compilation Compilation)> AnalyzeProject(Buildalyzer.ProjectAnalyzer projectAnalyzer)
        {
            ...
            var trees = await Task.WhenAll(
                csFiles.Select(async x => SyntaxFactory.ParseSyntaxTree(await File.ReadAllTextAsync(x), new CSharpParseOptions(), x))
            );
            ...
            var compilationFile = Path.Combine(Path.GetTempPath(), $"{Path.GetFileNameWithoutExtension(projectAnalyzer.ProjectFile.Path)}.dll");
        ...
        (string File, Compilation Compilation)[] _compilations;

        readonly Dictionary<Compilation, IEnumerable<IMethodSymbol>> _extensionMethods = new Dictionary<Compilation, IEnumerable<IMethodSymbol>>();
        ...
        public MetadataItem[] ExtractMetadata(Action<ExtractMetadataOptions> configure = null)
        {
            ...
            {
                RoslynExtensionMethods = _extensionMethods
            };
            configure?.Invoke(options);

            return _compilations.Select(x => x.Compilation.ExtractMetadata(options: options)).ToArray();
        }
