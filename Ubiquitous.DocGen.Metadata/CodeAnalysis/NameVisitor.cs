...
namespace Ubiquitous.DocGen.Metadata.CodeAnalysis
{
    ...
    {
        static readonly Func<CSharpNameVisitor>[] CsCreators =
            Enumerable
                .Range(0, (int) NameOptions.All + 1)
                .Select<int, Func<CSharpNameVisitor>>(x => () => new CSharpNameVisitor((NameOptions) x))
                .ToArray();

        public static string GetName(this Func<CSharpNameVisitor> getVisitor, ISymbol symbol)
        {
        ...
        public static Func<CSharpNameVisitor> GetCSharp(NameOptions option)
        {
    ...
    {
        None                       = 0, UseAlias              = 1, WithNamespace = 2, WithTypeGenericParameter = 4, WithParameter = 8, WithType = 16,
        WithMethodGenericParameter = 32, WithGenericParameter = WithTypeGenericParameter | WithMethodGenericParameter,
        Qualified                  = WithNamespace                                       | WithType, All = UseAlias | WithNamespace |
            WithTypeGenericParameter                                                                                | WithParameter | WithType |
            WithMethodGenericParameter,
    }

    public sealed class CSharpNameVisitor : SymbolVisitor
    {
        readonly NameOptions   _options;
        readonly StringBuilder _sb = new StringBuilder();

        CSharpNameVisitor Append(string text)
        {
            _sb.Append(text);
            return this;
        }
        ...
        public CSharpNameVisitor(NameOptions options) => _options = options;

        public override void VisitNamedType(INamedTypeSymbol symbol)
        {
            if (OptionsMatch(NameOptions.UseAlias) && TrySpecialType(symbol))
            {
                return;
            }
            ...
            if (OptionsMatch(NameOptions.WithTypeGenericParameter) && symbol.TypeParameters.IsNotEmpty())
            {
            ...
            static bool InGlobalNamespace(ISymbol symbol) => symbol.ContainingNamespace == null || symbol.ContainingNamespace.IsGlobalNamespace;
        }
        ...
        public override void VisitMethod(IMethodSymbol symbol)
        {
            ...
            {
                AppendParameters(
                    symbol.Parameters, "(", ")", inBetween: x =>
                    {
                        if (symbol.MethodKind != MethodKind.Conversion || symbol.ReturnsVoid) return;
        ...
        static readonly Dictionary<SpecialType, string> SpecialTypeMap = new Dictionary<SpecialType, string>
        {
        ...
        void AppendParameters<T>(ImmutableArray<T> parameters, string start, string end, Action<T> accept = null, Action<T> inBetween = null)
            where T : ISymbol
        {
            if (accept == null)
        ...
        void AppendParameters(int typeParameterCount, string start, string end) => Append(start + new string(',', typeParameterCount) + end);
    }
}