                ...
                AttributeArgumentList(
                    SeparatedList(
                        attr.ConstructorArguments
                            ...
                            .Where(expr => expr.HasValue)
                            .Select(x => AttributeArgument(x.Value))
                            .Concat(
                                attr.NamedArguments
                                    .Select(
                                        item => (item.Key, Expression: item.Value.GetLiteralExpression())
                                    )
                                    .Where(t => t.Expression.HasValue)
                                    .Select(
                                        t => AttributeArgument(
                                            NameEquals(IdentifierName(t.Key)),
                                            null,
                                            t.Expression.Value
                                        )
                                    )
