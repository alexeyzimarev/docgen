...
namespace DocGen.Metadata.Models
{
    public class ReferenceItem
    {
        ...
        public List<LinkItem> Parts { get; set; } = new List<LinkItem>();
        ...
        {
            Debug.Assert(
                source == null || target == null || Equals(source, target),
                $"{source} {target}"
            );
            return (source ?? target)!;
        }
        ...
        public void Merge(ReferenceItem other)
        {
            ...
            if (IsDefinition == true) CommentId = MergeCommentId(other.CommentId, CommentId);
            ...
            if (other.Parts != null && Parts != null && other.Parts.Count > 0)
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < other.Parts.Count; i++)
                    {
                        ...
                        Debug.Assert(
                            other.Parts[i].DisplayQualifiedName == Parts[i].DisplayQualifiedName
                        );
