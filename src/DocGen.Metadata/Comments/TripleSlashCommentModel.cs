...
namespace DocGen.Metadata.Comments
{
    public class TripleSlashCommentModel
    {
        ...
        public void CopyInheritedData(TripleSlashCommentModel src)
        {
            ...
            if (Exceptions == null && src.Exceptions != null) Exceptions = src.Exceptions.Select(e => e.Clone()).ToList();

            if (Sees == null && src.Sees != null) Sees = src.Sees.Select(s => s.Clone()).ToList();

            if (SeeAlsos == null && src.SeeAlsos != null) SeeAlsos = src.SeeAlsos.Select(s => s.Clone()).ToList();

            if (Examples == null && src.Examples != null) Examples = new List<string>(src.Examples);

            if (Parameters == null && src.Parameters != null) Parameters = new Dictionary<string, string>(src.Parameters);

            if (TypeParameters == null && src.TypeParameters != null) TypeParameters = new Dictionary<string, string>(src.TypeParameters);
        }
        ...
        static string GetValue(string name, Dictionary<string, string> dictionary)
            => (dictionary == null
                ? null
                : dictionary.TryGetValue(name, out var description)
                    ? description
                    : null)!;
        ...
        void ResolveCodeSource(XNode doc)
        {
            foreach (var node in doc.XPathSelectElements("//code"))
            {
                ...
                if (source == null || string.IsNullOrEmpty(source.Value)) continue;
        ...
        static void ResolveCodeSource(XElement element, string source, string? region)
        {
            ...
            foreach (var line in File.ReadLines(source))
            {
                if (!string.IsNullOrEmpty(region))
                {
                    ...
                    if (regionCount > 0) builder.AppendLine(line);
                }
                else
                {
        ...
        static string NormalizeXml(string xml, int parentIndex)
        {
            ...
            foreach (var line in lines)
            {
                ...
                else
                {
                    ...
                    if (line[index] == '<') parentIndex = index;
        ...
        static string GetInnerXml(XPathNavigator node)
        {
            ...
            do tw.WriteNode(node, true); while (node.MoveToNext());
