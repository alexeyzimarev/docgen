...
namespace Ubiquitous.DocFx.Markdown.Visitors
{
    ...
    public enum NameOptions
    {
        ...
        All = UseAlias | WithNamespace | WithTypeGenericParameter | WithParameter | WithType |
    ...
    public sealed class CSharpNameVisitor : SymbolVisitor
    {
        ...
        public override void VisitNamedType(INamedTypeSymbol symbol)
        {
            ...
            if (symbol.IsTupleType)
            {
                ...
                {
                    AppendParameters(symbol.TupleElements, "(", ")",
                        x => x.Type.Accept(this),
                        x => { Append(" ").Append(x.Name); }
                    );
                }
            }
            else
            ...
            if (OptionsMatch(NameOptions.WithTypeGenericParameter) && symbol.TypeParameters.NotEmpty())
            {
                if (symbol.TypeArguments != null && symbol.TypeArguments.NotEmpty())
                {
        ...
        bool OptionsMatch(NameOptions options)
        ...
        public override void VisitMethod(IMethodSymbol symbol)
        {
            ...
            switch (symbol.MethodKind)
            {
                ...
                case MethodKind.UserDefinedOperator:
                    Append(symbol.Name.StartsWith("op_", StringComparison.Ordinal)
            ...
            {
                AppendParameters(symbol.Parameters, "(", ")", inBetween: x =>
                {
        ...
        void AppendParameters<T>(
            ImmutableArray<T> parameters, string start, string end, Action<T> accept = null, Action<T> inBetween = null
        ) where T : ISymbol
        {
            if (accept == null)
